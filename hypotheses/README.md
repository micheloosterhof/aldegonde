...
| Word boundaries | Preserved | — | Convention: a line break is NOT a word break (words wrap across lines); merged, the clean corpus has 2,928 words (2,953 with the solved AN END page included), mean 4.42 runes (solved pages: 4.01, Parable: 4.75 — the MEAN is within the author's stylistic range). The SHAPE is **not** English-like: 2-rune words are 15.9% against ~24% on two independent references, z ≈ −10, confined to that one length class — see `two-rune-deficit.md`. Sentences run much longer than solved pages (17.4 vs 8.0 words per '.' mark on the clean corpus, p=4e-6; per *qualifying* sentence the same signal reads 19.7 vs 8.9 — see `word-length-keystream-and-boundaries.md`) |
