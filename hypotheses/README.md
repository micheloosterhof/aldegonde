...
| Doublet x lag-5 interaction (symbol overlap, copied doublets, nearest-distance, rate-around-doublets, shared words) | All flat (\|z\| <= 0.9; 0 copied doublets) | — | The two anomalies are statistically independent; they share only the number 5; see `experiments/doublet_lag5_interaction.py` |
| Joint two-tap split test: mean nIoC of C(n) given (C(n-1), C(n-5)) | 1.028 (= 29/28 no-doublet correction) | Deterministic f(P,C1,C5): ~1.7 | Excludes EVERY deterministic two-tap autokey (product forms, interpolation, any constants/operations); see `product-form-autokey.md`, `experiments/product_form_ciphers.py` |
| Excision test: lag-5 stats after deleting the 106 paired-event copy targets (0.82% of glyphs) | mono 375, d1 0, d4 0 (all at/below chance) | — | The ENTIRE lag-5 phenomenon lives in 106 glyphs; the residual 99.2% is structureless in that channel; see `experiments/lp_battery22.py` |
| Linear annihilator scan: C[i]-a*C[i-j1]-b*C[i-j2] over 8 lag sets x GF(29)^2 (6,272 combos) | Max chi2 60.2 (threshold 63) | Linear-recurrence keystream: its annihilator combo lights up (calibrated: lagged-Fibonacci sim scores 67.9 at exactly its taps/coeffs) | The keystream satisfies NO low-order linear recurrence with taps <= 6 — incl. lagged Fibonacci {1,5}; see `experiments/lp_battery22.py` |
| Seeded conventional PRNGs as keystream (battery 21: LCG glibc/MINSTD/RANDU/NR, xorshift32, MT19937, SHA256-CTR, RC4 x 33 Cicada seeds/passphrases, strict + null-deletion alignment) | All flat (corpus max z 3.1/612 tests; page max 3.5 vs threshold 4.8/33,660 tests) | — | No standard generator with an obvious seed; the base stream stays indistinguishable from a pad; see `experiments/lp_battery21.py` |
