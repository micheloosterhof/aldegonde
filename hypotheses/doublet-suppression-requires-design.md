...
| **Feedback** (`f_{j+1}` depends on `c_j`) | adjacent positions coupled by construction | **excluded empirically** — each group sharing a previous rune would be enciphered by one fixed map, so grouped IoC must read ~1.78; measured within words 1.0227 / 0.9979 / 1.0060 at lags 1/2/5. Additive feedback separately dead (delta IoC 1.0246) |
...
Homophonic is the only class where the suppression costs nothing rather than being
designed, and it fails *only* on alphabet size. It would revive if the ciphertext
