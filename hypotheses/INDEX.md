...
Files are tagged `type: observation` or `type: hypothesis` in frontmatter.

### Observations (18) — measured features of the corpus

| File | Feature | Status |
|------|---------|--------|
| [bigram-ioc.md](bigram-ioc.md) | Off-Diagonal Bigrams are Uniform | confirmed (characterization) |
| [collision-hunt-single-constraint.md](collision-hunt-single-constraint.md) | The Ciphertext Yields One Cross-Word Constraint (DJU-BEI); the Wiring is Starved | confirmed (characterization) for the repeat census itself. The |
...
### Hypotheses (38) — proposed mechanisms
...
| [per-word-related-alphabets.md](per-word-related-alphabets.md) | Per-Word Related-Alphabet Cipher (5 Alphabets, Bigram-Dodging Step) | plausible — superseded by `length-clocked-walk.md`, which |
