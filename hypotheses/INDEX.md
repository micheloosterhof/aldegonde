...
| [dual-autokey-lag1-lag5.md](dual-autokey-lag1-lag5.md) | Dual Autokey: Lag-1 Ciphertext plus Lag-5 Plaintext | partial (Aug 2026, Michel's proposal): escapes the grouped-IoC test that kills pure ciphertext autokey (1.046 vs 1.756), and gets doublet suppression and boundary-blindness from structure with no tuned permutation -- but floors ~1.4x short on depth (0.86% vs 0.63%) and produces no d5 echo |
