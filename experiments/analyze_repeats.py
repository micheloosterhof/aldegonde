...
def extract_raw_context(
    raw: str, rune_pos: int, before: int = 20, after: int = 20
) -> str:
...
def find_repeats(
    indices: list[int], min_n: int = 5, max_n: int = 10
) -> dict[int, list[tuple[tuple[int, ...], list[int]]]]:
...
def analyze_repeat_context(
    ...
    for occ_idx, pos in enumerate(positions):
        ...
        word_engs = ["".join(ENG[c3301.r2i(c)] for c in w) for w in words]
...
def state_propagation_analysis(
    ...
    print("    Plaintext DIFFERS (different C[i-1], same C[i])")

    print("\n  If state[i] depends on P[i-1] (prev plaintext):")
    print(f"    state_a[1] = f(P_a[0]) = f({ENG[p_a0]})")
    print(f"    state_b[1] = f(P_b[0]) = f({ENG[p_b0]})")
    print("    States differ at position 1 => P[1] may also differ")
    print("    => Difference can propagate through entire repeat")
    print("    For same ciphertext with different states:")
    print("    P_a[j] * k(state_a[j]) = P_b[j] * k(state_b[j]) at each position j")
    print("    This is a chain of equations in GF(29) that must hold simultaneously.")

    print("\n  If state[i] depends on C[i-1] only (disproved single-autokey):")
    print("    From position 1 onwards, C[i-1] matches => P matches")
    print("    Only position 0 differs")

    print("\n  If state[i] depends on C[i-1] and C[i-2]:")
    prev2_a = indices[pos_a - 2] if pos_a > 1 else 0
    prev2_b = indices[pos_b - 2] if pos_b > 1 else 0
    print(
        f"    C[i-2]: {ENG[prev2_a]} vs {ENG[prev2_b]} — "
        f"{'MATCH' if prev2_a == prev2_b else 'DIFFER'}"
    )
    if prev2_a != prev2_b:
        print("    Positions 0 and 1 may have different state => different P")
        print("    From position 2 onwards: C[i-1] and C[i-2] both match => P matches")
    ...
    print(
        f"    Running sums at start: {sum_a} vs {sum_b} — "
        f"{'MATCH' if sum_a == sum_b else 'DIFFER'}"
    )
    if sum_a != sum_b:
        diff = (sum_a - sum_b) % N
        print(f"    Difference: {diff}")
        print("    Running sum diverges by a constant throughout the repeat")
        print(
            "    Every position has different state => every position may have different P"
        )
...
def main() -> None:
    ...
    for ngram_len in sorted(repeats.keys()):
        ...
        for _gram, positions in grams:
            analyze_repeat_context(indices, raw, positions, ngram_len)
            if len(positions) == 2:
                state_propagation_analysis(
                    indices, positions[0], positions[1], ngram_len
                )
    ...
    for pos, label in [(pos_a, "A"), (pos_b, "B")]:
        ...
        print(
            f"    Runes:   {ctx_runes[:marker]}[{ctx_runes[marker : marker + 7]}]{ctx_runes[marker + 7 :]}"
        )
        print(
            f"    English: {ctx_eng[:marker]}[{ctx_eng[marker : marker + 7]}]{ctx_eng[marker + 7 :]}"
        )
    ...
    print("  under Beaufort autokey with various offsets:")
    print("  These 6 runes decrypt identically at both occurrences")
    print("  regardless of offset (C[i-1] matches from position 1).\n")
    ...
    for offset in range(N):
        ...
        any(ENG[p] in ("E", "T", "A", "O", "I", "N", "S", "H", "R") for p in pt)
        marker = (
            " <--"
            if eng.replace("TH", "T").replace("NG", "N").replace("EA", "E").isalpha()
            else ""
        )
