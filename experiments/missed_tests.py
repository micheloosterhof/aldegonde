...
print(
    "top 6 by full-table z:",
    [(lag, f"{z:+.2f}/{zod:+.2f}") for z, zod, lag in results[:6]],
)
...
for length in range(4, 11):
    ...
    md, sd_, mdup, sdup = isomorph.random_isomorph_statistics(n, length, samples=8)
    z = (duplicate - mdup) / sdup if sdup > 0 else 0.0
    print(
        f"  len {length:2d}: duplicates {duplicate:6d} vs random "
...
print(
    "top 5:",
    [(c3301.CICADA_ENGLISH_ALPHABET[r], p, f"{v:.3f}") for v, r, p in best[:5]],
)
...
print("top 4:", [(c3301.CICADA_ENGLISH_ALPHABET[r], f"{v:.3f}") for v, r in best4[:4]])
...
for name, series in (("idx", idx.astype(float)), ("GP", gpv)):
    ...
    sig = 1 / sqrt(n)
    print(
        f"  {name}: top |r| {[(lag, f'{r_:+.4f}') for _, lag, r_ in rs[:3]]} "
...
print(
    "  DFT top peaks: "
    + ", ".join(f"period {n / (k + 1):.1f} power {ps[k] / expp:.1f}x" for k in top)
)
mx = ps.max() / expp
print(
    f"  max peak {mx:.1f}x mean; null max over {len(ps)} bins ~ {np.log(len(ps)):.1f}x"
)
...
print(
    "smallest p-values:",
    [(c3301.CICADA_ENGLISH_ALPHABET[r], f"{p:.4f}") for p, r in worst[:4]],
...
secs = [
    s
    for s in ("".join(x for x in t if x in AB) for t in raw.split("$")[:10])
    if len(s) > 200
]
vals = []
...
print(
    f"range {vals[-1][0]:.3f} .. {vals[0][0]:.3f} "
...
for length in (7, 9, 10):
    ...
    for _ in range(8):
        m = gen_suppressed(n)
        _, d2 = isomorph.isomorph_statistics(isomorph.isomorph_distribution(m, length))
        sims.append(d2)
    mu = statistics.mean(sims)
    sd_ = statistics.stdev(sims)
    print(
        f"  len {length:2d}: LP {duplicate} null {mu:.1f}+/-{sd_:.1f} "
