...
def ks(a, b):
    ...
    D = max(
        abs(
            sum(1 for x in sa if x <= v) / len(sa)
            - sum(1 for x in sb if x <= v) / len(sb)
        )
        for v in allv
    )
    lam = (D + 0) * math.sqrt(len(sa) * len(sb) / (len(sa) + len(sb)))
    p = 2 * sum(
        (-1) ** (k - 1) * math.exp(-2 * k * k * lam * lam) for k in range(1, 101)
    )
...
def row(name, ws, ss):
    ...
    print(
        f"{name:>10}: {len(ws):4d} words, mean {statistics.mean(ws):.2f}, short {short:.0%}, "
...
print(
    f"    solved: {len(solved_w)} words, mean {statistics.mean(solved_w):.2f}, "
