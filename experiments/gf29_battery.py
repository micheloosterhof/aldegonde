...
def main() -> None:
    ...
    for mod, name in (
        ...
        for s in range(1, nl // 2):
            ...
            if abs(z) > abs(zmax):
                zmax, smax = z, s
        print(
            f"  L mod {mod:2d} ({name}): marginal chi2={stat:.1f} p={pm:.3f}; "
    ...
    print(
        f"  Legendre runs: {runs} (exp {mu:.0f}, z={(runs - mu) / math.sqrt(var):+.2f})"
    )
    ...
    print(
        f"  {ntests} tests; significant at p<1e-4: "
    ...
    for stat, a_, b_, s1, s2 in worst:
        print(
            f"    lags ({a_},{b_}) signs ({'+' if s1 == 1 else '-'},"
