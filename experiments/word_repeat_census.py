...
def extra_pairs(words: list[tuple[int, ...]], minlen: int) -> int:
    ...
    return sum(
        len(v) * (len(v) - 1) // 2 for v in repeat_classes(words, minlen).values()
    )
...
def consecutive_runs(words: list[tuple[int, ...]]) -> list[tuple[int, int, int]]:
        ...
        for a in idxs:
            for b in idxs:
                if b <= a:
                    continue
                run = 0
                while a + run < n and b + run < n and words[a + run] == words[b + run]:
                    run += 1
                if run >= 2:
    ...
    for a, b, r in found:
        if not any(
            a2 < a and b2 < b and a - a2 == b - b2 and r2 - (a - a2) >= r
...
def main() -> None:
    ...
    print(
        f"null: doublet_shuffle at the observed rate {rate:.5f}, "
    ...
    for a, b, r in obs_runs:
        ...
        print(
            f"  words {a},{b}: run of {r} ({runes} runes) "
