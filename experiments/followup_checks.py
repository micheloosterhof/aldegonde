...
def main() -> None:
    ...
    for window in (5, 10, 25):
        ...
        print(f"   pairs within {window:>2} words: obs {obs} "
    ...
    pd = [i for i in range(len(plaintext) - 1)
          if plaintext[i] == plaintext[i + 1]]
    close_pt = sum(1 for a in range(len(pd)) for b in range(a + 1, len(pd))
                   if pd[b] - pd[a] <= 5)
    scale = n / len(plaintext)
    pred = close_pt * scale * (1 / 5) ** 2
    dbl = [i for i in range(n - 1) if stream[i] == stream[i + 1]]
    close_ct = sum(1 for a in range(len(dbl)) for b in range(a + 1, len(dbl))
                   if dbl[b] - dbl[a] <= 5)
    print(f"\nB. plaintext doublet pairs with gap <= 5 in the register: "
          f"{close_pt} per {len(plaintext)} runes")
    print(f"   hold-model predicted close ciphertext doublet pairs: "
