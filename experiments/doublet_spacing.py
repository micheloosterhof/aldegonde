...
def main() -> None:
    ...
    print(
        f"n={n} runes, D={D} doublets, rate={lam:.5f}, mean spacing 1/rate={1 / lam:.1f}"
    )
    print("\n--- GAP DISTRIBUTION ---")
    print(
        f"min {gaps.min()}  max {gaps.max()}  mean {gaps.mean():.1f}  "
        f"median {int(np.median(gaps))}  std {gaps.std():.1f}"
    )
    print(
        f"CV std/mean = {gaps.std() / gaps.mean():.3f}  (geometric/exponential -> 1.0)"
    )
    print(
        f"gaps below 6: {int((gaps < 6).sum())}  "
    ...
    try:
        ...
        print(
            f"  KS gaps vs exponential: D={ks.statistic:.3f} p={ks.pvalue:.3f} "
    ...
    print(
        f"  max-peak / mean-power = {peak_ratio:.2f}  "
    ...
    for label, m in [
        ...
        allm = {
            "i prime": isprime[:n],
            "i+1 prime": isprime[1 : n + 1],
            "i-1 prime": isprime[np.arange(-1, n - 1).clip(0)],
        }[label]
    ...
    chi = (
        (np.bincount(gaps % 29, minlength=29) - len(gaps) / 29) ** 2 / (len(gaps) / 29)
    ).sum()
    print(f"  gaps mod 29 chi-sq: {chi:.1f} (uniform if < ~41)")
    print(
        f"  phi(i) mod 29 mean at doublets: {(phi[pos] % 29).mean():.2f}  "
