...
def frag_fitness(frag):
    if len(frag) < 2:
        return 0.0
    return sum(BLL[frag[i], frag[i + 1]] for i in range(len(frag) - 1)) / (
        len(frag) - 1
    )
...
ntests = sum(
    2 * (n - len(to_runes(c)) + 1) for c in CRIBS if 3 <= len(to_runes(c)) <= 13
)
print(
    f"total placements tested: {ntests} -> Bonferroni z ~ {(2 * math.log(ntests)) ** 0.5:.1f}"
)
...
for crib in ("THE", "WITHIN", "PRIMES", "CIRCUMFERENCE"):
    ...
    print(
        f"{crib}: arithmetic-key placements: {len(arr)} (expect ~{2 * (n - L + 1) / N ** (L - 2):.2g})"
    )
