...
for i, s in enumerate(secs):
    ...
    print(f"  sec {i}: {r:5d} runes, {m:3d} marks, rate {m/r:.4f}, expected {exp:.1f}{note}")
print(f"  total: {tot_r} runes, {tot_m} marks, rate {tot_m/tot_r:.4f} (1/{tot_r/tot_m:.0f})")
...
def chi2(d, n_marks):
    exp = n_marks / 29
    return sum((d.get(r, 0) - exp) ** 2 / exp for r in set(list(d.keys()))) , len(d)
for name, d in (("rune before mark", ctx_before), ("rune after mark", ctx_after)):
    n = sum(d.values())
    exp = n / 29
    c2 = sum((v - exp) ** 2 / exp for v in d.values()) + (29 - len(d)) * exp
    print(f"{name}: n={n}, distinct {len(d)}/29, chi2 {c2:.1f} (df 28, crit@0.05 ~41.3)")
