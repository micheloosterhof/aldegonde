...
**Deterministic collision-restep decryption** (`experiments/lp_battery13.py`). The
...
**Per-page keystream offset brute force** (`experiments/lp_battery14.py`). For each
...
**Crib dragging** (`experiments/lp_battery15.py`). For an additive cipher, a correctly
...
scripts with reusable attack/simulation code are kept in `experiments/`
(run from the repository root; they read `data/page0-58.txt`):

```
python experiments/lp_structure.py     # parsing + baseline (shared parser the batteries import)
python experiments/lp_battery9.py      # prefix attack + Viterbi drift attack
python experiments/lp_battery11.py     # mechanism kill-table (20 simulated mechanisms)
python experiments/lp_battery13.py     # deterministic collision-restep decryption
python experiments/lp_battery14.py     # per-page keystream offset brute force (16.5M trials)
python experiments/lp_battery15.py     # crib drag + key-fragment structure test
```

The remaining batteries (1-8, 10, 12, 16-18: lag/period scans, doublet
anatomy, anchored tests, Monte-Carlo nulls, difference-domain attacks,
doublet-removed streams) were straightforward counting/chi-square loops;
their results are fully recorded in the sections above and they can be
rewritten from the descriptions if a re-run is ever needed.
