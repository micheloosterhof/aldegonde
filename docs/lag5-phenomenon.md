...
- **Period-5 shift cipher under any ciphertext-computable interruptor
  schedule** (the loophole a plain kappa test leaves open — the solved
  "AN END" page used exactly this trick): a per-page chi-square attack over
  61 key schedules (every single-rune skip:R and reset:R interruptor, plus
  word- and sentence-restart schedules; 6,710 runs; positive control cracks
  a 95-rune synthetic at z ≈ +7) finds no page decrypting under any of
  them, and a corpus-wide coset-IoC sweep excludes period-5 polyalphabetic
  (incl. Quagmire) under the same schedules. Scripts:
  `examples/lp_lag5_attack.py`, `examples/lp_lag5_analysis.py`.
...
either designer aesthetic or coincidence, not one mechanism. One more
appearance of 5: the corpus's only repeated 6-gram (the word-aligned
ᛞᛄᚢ-ᛒᛖᛁ phrase, see `hypotheses/repeated-phrase-dju-bei.md`) sits at a
distance of 6,395 runes = 5 × 1279 (both prime).
...
A scoring pitfall for anyone testing candidate decryptions: n-gram fitness
scores must be compared against a **doublet-preserving** null. Scoring
