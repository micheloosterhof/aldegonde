...
from aldegonde.validation import validate_text_sequence, validate_key_length, validate_tabula_recta
...
def pasc_encrypt(
    ...
    plaintext_seq = list(plaintext) if not isinstance(plaintext, Sequence) else plaintext
    ...
    try:
        for e, k in zip(plaintext_seq, cycle(keyword)):
            if k not in tr:
                raise KeyError(f"Key symbol '{k}' not found in tabula recta", key=k, cipher_type="polyalphabetic")
            if e not in tr[k]:
                raise CipherError(f"Plaintext symbol '{e}' not found in tabula recta for key '{k}'", cipher_type="polyalphabetic")
            yield tr[k][e]
    except Exception as exc:
        if isinstance(exc, (KeyError, CipherError)):
            raise
        raise CipherError(f"Encryption failed: {exc}", cipher_type="polyalphabetic") from exc
...
def pasc_encrypt_interrupted(
    ...
    if ciphertext_interruptor is None and plaintext_interruptor is None:
        raise InvalidInputError("At least one interruptor (ciphertext or plaintext) must be specified")
    ...
    try:
        ...
        for e in plaintext_seq:
            ...
            if key not in tr:
                raise KeyError(f"Key symbol '{key}' not found in tabula recta", key=key, cipher_type="polyalphabetic")
            if e not in tr[key]:
                raise CipherError(f"Plaintext symbol '{e}' not found in tabula recta for key '{key}'", cipher_type="polyalphabetic")
        ...
        if isinstance(exc, (KeyError, CipherError, InvalidInputError)):
            raise
        raise CipherError(f"Interrupted encryption failed: {exc}", cipher_type="polyalphabetic") from exc
...
def reverse_tr(tr: TR[T]) -> TR[T]:
    ...
    if not isinstance(tr, dict):
        raise InvalidInputError(f"Tabula recta must be a dictionary, got {type(tr).__name__}")
    
    output: TR[T] = defaultdict(dict)
    for keyword in tr:
        if not isinstance(tr[keyword], dict):
            raise InvalidInputError(f"Inner tabula recta for key '{keyword}' must be a dictionary")
        
        for k, v in tr[keyword].items():
            output[keyword][v] = k
        
        if len(output[keyword]) != len(tr[keyword]):
            raise CipherError(f"Tabula recta is ambiguous for key '{keyword}' - contains duplicate values", cipher_type="polyalphabetic")
