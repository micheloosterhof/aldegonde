...
def print_isomorph_statistics(seq: Sequence[object], *, trace: bool = False) -> None:
    ...
    for length in range(startlength, endlength + 1):
        ...
        try:
            print(
                f"{distinct:5d} (avg: {avgdistinct:8.2f}, S={(distinct - avgdistinct) / stdevdistinct:+5.2f}σ)",
                end=" ",
            )
        ...
        try:
            print(
                f"duplicate: {duplicate:5d} (avg: {avgduplicate:8.2f} S={(duplicate - avgduplicate) / stdevduplicate:+5.2f}σ) ",
            )
        except ZeroDivisionError:
