...
from aldegonde.stats.compare import chisquare, gtest, quadgramscore
...
def test_chisquare() -> None:
    assert chisquare(am, nz) == 0.0
    assert chisquare(a, a) == 1.0
...
def test_quadgramscore() -> None:
    ...
    assert quadgramscore("test") < -11.6
    assert quadgramscore("test") > -11.7
